...
export function activate(context: vscode.ExtensionContext) {
	const disposable = vscode.commands.registerCommand('extension.copyInclude', async (uri: string) => {
		if (!uri && vscode.window.activeTextEditor) {
			uri = vscode.window.activeTextEditor.document.uri.toString();
		}
		if (!uri) {
			vscode.window.showErrorMessage('Cannot copy include, no file is opened');
			return;
		}
		const headerPath = getHeaderFilePath(uri);
		if (!headerPath) {
			vscode.window.showErrorMessage('Unable to determine header path: ' + uri);
			return;
		}
